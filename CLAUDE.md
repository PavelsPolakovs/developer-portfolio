...
3. Включить squash auto-merge и ждать, пока PR замёрджится (все 5 CI-проверок должны пройти). При ручном опросе проверять отсутствие `pending` строк — не наличие `pass`.
